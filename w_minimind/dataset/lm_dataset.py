...
class PretrainDataset(Dataset):
    def __init__(self, data_path, tokenizer, max_length=512):
        ...
        self.max_length = max_length #输入给GPU的最大长度，超过这个长度的文本会被截断，保证训练效率和显存使用合理
        # 使用 HuggingFace datasets 的惰性加载，避免一次性读入大文件
        self.samples = load_dataset("json", data_files=data_path, split="train")
    #len方法返回数据集的样本数量，getitem方法根据索引获取样本并进行处理，返回 input_ids、labels 和 attention_mask 三个张量，供模型训练使用。
    def __len__(self):
        return len(self.samples)
    
    #——我们拿到的是，jsonl的每一行
    #tokenier把文本转化成iput_id
    #需要加上EOS,BOS,以及PAD的处理，保证输入模型的格式正确
    #需要自行编写labels，防止pad位置参与loss计算
    #需要编写attention_mask，屏蔽掉pad位置的注意力计算,哪些位置有效
    #需要输出的是，input_ids,attetion_mask,labels
    def __getitem__(self, index):
        ...
        tokens = self.tokenizer(
            ...
            truncation=True,# 超过 max_length 的部分直接截断，保证输入长度不超过模型的最大处理能力
        ).input_ids
...
class RLAIFDataset(Dataset):
    ...
    def create_chat_prompt(self, conversations):
        ...
        #加上引导语，告诉模型"现在开始生成"，让模型在 RL 训练时能正确识别续写位置，提升生成质量
        prompt = post_processing_chat(prompt)
