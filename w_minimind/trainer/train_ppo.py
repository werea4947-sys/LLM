...
from model.MyModel import MyMindConfig, MyMindForCausalLM  # MiniMind模型
...
class CriticModel(MyMindForCausalLM):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="MyMind PPO (Proximal Policy Optimization)"
    )
    ...
    parser.add_argument(
        "--wandb_project", type=str, default="MyMind-PPO", help="wandb项目名"
    )
    ...
    if args.use_wandb and is_main_process():
        ...
        wandb_run_name = f"MyMind-PPO-Epoch-{args.epochs}-BS-{args.batch_size}-LR-{args.learning_rate}"
        wandb.init(
            project=args.wandb_project, name=wandb_run_name, id=wandb_id, resume=resume
        )
    # ========== 5. 初始化模型和数据 ==========
    # 📚 PPO模型架构
    base_weight = "reason" if args.reasoning == 1 else "sft"
