"""
MyMind LoRA (Low-Rank Adaptation) 微调脚本  # ！修正：原MiniMind改为MyMind
...
# MyMind相关组件
from model.MyModel import MyMindConfig  # 模型配置
...
def train_epoch(epoch, loader, iters, lora_params, start_step=0, wandb=None):
    ...
    for step, (input_ids, labels, attention_mask) in enumerate(
        ...
        with autocast_ctx:
            ...
            # SFT总损失 = 主任务loss + 辅助loss（仅MoE时可能存在）
            aux_loss = getattr(res, "aux_loss", None)
            if aux_loss is None:
                aux_loss = res.loss.new_zeros(())
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="MyMind LoRA Fine-tuning")
    ...
    parser.add_argument(
        "--wandb_project", type=str, default="MyMind-LoRA", help="wandb项目名"
    )
    ...
    # 📚 模型配置初始化
    # MyMindConfig: 定义模型的超参数，如隐藏维度、层数、是否使用MoE
    ...
    if args.use_wandb and is_main_process():
        ...
        # 📚 实验名称生成
        # 包含关键参数，便于识别不同的实验配置
        wandb_run_name = f"MyMind-LoRA-{args.lora_name}-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LR-{args.learning_rate}"
        wandb.init(
